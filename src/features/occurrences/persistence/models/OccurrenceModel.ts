...
const OccurrenceSchema = new Schema<IOccurrenceDocument>({
  type: { type: String, enum: ['student_not_in_class','manual_note','invalid_qr','duplicate_scan'], required: true, index: true },
  teacher_id: { type: Number, required: true, index: true },
  student_cpf: { type: String, index: true, sparse: true },
  class_id: { type: Number, index: true, sparse: true },
  description: { type: String, required: true, maxlength: 1000 },
  notified_to_institution: { type: Boolean, default: false, index: true }
},{ timestamps: { createdAt: 'created_at', updatedAt: false }, collection: 'occurrences' });
