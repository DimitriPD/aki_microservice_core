...
import { EventRepository, EventFilters, PaginationOptions, PaginatedResult } from '../domain/repositories/EventRepository';
...
export class EventRepositoryImpl implements EventRepository {
  ...
  async findAll(filters: EventFilters, pagination: PaginationOptions): Promise<PaginatedResult<Event>> {
    ...
    const [documents, total] = await Promise.all([
      EventModel.find(query).sort({ start_time: -1 }).skip(skip).limit(pagination.size),
      EventModel.countDocuments(query)
    ]);
    return { items: documents.map(doc => this.mapToDomain(doc)), meta: { page: pagination.page, size: pagination.size, total } };
  }

  async findOverlappingEvents(classId: number, startTime: Date, endTime: Date, excludeId?: EventId): Promise<Event[]> {
    const query: any = { class_id: classId, status: { $ne: 'canceled' }, $and: [ { start_time: { $lt: endTime } }, { end_time: { $gt: startTime } } ] };
    if (excludeId) query._id = { $ne: excludeId.toString() };
  ...
  async update(event: Event): Promise<Event> {
    ...
    const eventData = this.mapToDocument(event);
    const updatedDocument = await EventModel.findByIdAndUpdate(event.id.toString(), eventData, { new: true, runValidators: true });
    if (!updatedDocument) throw new NotFoundError('Event not found');
  ...
  private mapToDocument(event: Event): Partial<IEventDocument> {
    const doc: any = { class_id: event.classId, teacher_id: event.teacherId, start_time: event.startTime, end_time: event.endTime, location: { latitude: event.location.latitude, longitude: event.location.longitude }, status: event.status, updated_at: new Date() };
    if (event.qrToken) doc.qr_token = event.qrToken;
    return doc;
  }

  private mapToDomain(document: IEventDocument): Event {
    const eventData: any = { id: new EventId((document as any)._id.toString()), classId: document.class_id, teacherId: document.teacher_id, startTime: document.start_time, endTime: document.end_time, location: new Location(document.location.latitude, document.location.longitude), status: document.status, createdAt: document.created_at, updatedAt: document.updated_at };
    if (document.qr_token) eventData.qrToken = document.qr_token;
