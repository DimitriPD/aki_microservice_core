...
import { AttendanceId, AttendanceStatus } from '../../domain/value-objects/CommonTypes';
...
import { AttendanceModel, IAttendanceDocument } from '../database/models/AttendanceModel';
...
export class AttendanceRepositoryImpl implements IAttendanceRepository {
  ...
  async findById(id: AttendanceId): Promise<Attendance | null> {
    const document = await AttendanceModel.findById(id.toString());
  ...
  async findByEventAndStudent(eventId: string, studentId: number): Promise<Attendance | null> {
    const document = await AttendanceModel.findOne({
      event_id: eventId,
  ...
  async update(attendance: Attendance): Promise<Attendance> {
    ...
    const attendanceData = this.mapToDocument(attendance);
    const updatedDocument = await AttendanceModel.findByIdAndUpdate(
  ...
  async delete(id: AttendanceId): Promise<void> {
    const result = await AttendanceModel.findByIdAndDelete(id.toString());
    if (!result) {
      throw new NotFoundError('Attendance not found');
    }
  }

  async exists(id: AttendanceId): Promise<boolean> {
    const count = await AttendanceModel.countDocuments({ _id: id.toString() });
    return count > 0;
  }

  async existsByEventAndStudent(eventId: string, studentId: number): Promise<boolean> {
  ...
  private buildQuery(filters: AttendanceFilters): Record<string, any> {
    const query: Record<string, any> = {};

    if (filters.eventId) {
      query.event_id = filters.eventId;
    }
  ...
  private mapToDocument(attendance: Attendance): Partial<IAttendanceDocument> {
    return {
      event_id: attendance.eventId as any,
      ...
      validation: attendance.validation ? {
        within_radius: attendance.validation.withinRadius,
        distance_meters: attendance.validation.distanceMeters
      } : undefined,
  ...
  private mapToDomain(document: IAttendanceDocument): Attendance {
    return Attendance.fromPersistence({
      id: new AttendanceId((document as any)._id.toString()),
      eventId: document.event_id.toString(),
      ...
      validation: document.validation ? ValidationResult.create(
        document.validation.within_radius,
        document.validation.distance_meters
      ) : undefined,
