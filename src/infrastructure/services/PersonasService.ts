...
export class PersonasServiceImpl implements PersonasService {
  ...
  async getStudentByCpf(cpf: string): Promise<Student | null> {
    try {
      ...
      throw new ExternalServiceError('Failed to fetch student information', 'PersonasService');
    }
  }
  ...
  async isStudentInClass(studentId: number, classId: number): Promise<boolean> {
    try {
      ...
      logger.error('Error checking student class membership', { studentId, classId, error: error.message });
      throw new ExternalServiceError('Failed to verify student class membership', 'PersonasService');
    }
  }
  ...
  async getTeacherById(teacherId: number): Promise<Teacher | null> {
    try {
      ...
      if (process.env.NODE_ENV === 'development') return this.mockGetTeacherById(teacherId);
  ...
  async getClassById(classId: number): Promise<ClassSummary | null> {
    try {
      ...
      if (process.env.NODE_ENV === 'development') return this.mockGetClassById(classId);
  ...
  async isTeacherInClass(teacherId: number, classId: number): Promise<boolean> {
    try {
      ...
      if (process.env.NODE_ENV === 'development') return this.mockIsTeacherInClass(teacherId, classId);
      const response = await axios.get(`${this.baseUrl}/classes/${classId}/teachers`, {
        timeout: this.timeout,
        headers: { 'Content-Type': 'application/json' }
      });
  ...
  private mockGetStudentByCpf(cpf: string): Student | null {
    ...
    return mockStudents.find(s => s.cpf === cpf) || null;
  }
  private mockIsStudentInClass(studentId: number, classId: number): boolean {
    ...
    return valid.some(v => v.studentId === studentId && v.classId === classId);
  }
  ...
  private mockIsTeacherInClass(teacherId: number, classId: number): boolean {
    ...
    return valid.some(v => v.teacherId === teacherId && v.classId === classId);
  }
}
